...
def all_records(request):
    """
    End point for returning all the records in proper report sorted and consolidated
    """
...
def detail(request, file_id):
    """
    End point for returning a single supplied file ID records in proper report sorted and consolidated
    """
    if int(file_id) < 1:
        file_id = -1
...
def pay_upload(request):
    ...
    try:
        ...
        messages.error(
            request, 'Something went wrong with the file! File must be a csv file in this format time-report-42.csv.')
